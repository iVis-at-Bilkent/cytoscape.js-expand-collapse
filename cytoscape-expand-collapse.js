;(function () {
  ...
  var register = function (cytoscape) {
    ...
    var options = {
      rearrange: true,
      layoutOptions: { }, // for rearrange after expand/collapse
      fisheye: true,
    ...
    function setOptions(from) {
      var tempOpts = JSON.parse(JSON.stringify(options));
      for (var key in from)
    ...
    cytoscape('core', 'collapseAll', function (opts) {
      var cy = this.expandableNodes();
      var tempOptions = setOptions(opts);

      return cy.nodes(":parent").collapseAll(tempOptions);
    });

    // cy.expandAll(options)
    cytoscape('core', 'expandAll', function (opts) {
      var cy = this.collapsibleNodes();
      var tempOptions = setOptions(opts);

      return cy.nodes(":parent").expandAll();
    });
    ...
    cytoscape('collection', 'isCollapsible', function () {
      ...
      return (eles.data("expanded-collapsed") === "collapsed");
    });
    ...
    cytoscape('collection', 'isExpandable', function () {
      ...
      return !eles.isCollapsible() && eles.isParent();
    });
    ...
    var expandCollapseUtilities = {
      ...
      expandAllNodes: function (nodes, options) {//*//
        ...
        if (options.rearrange)
          cy.layout(options.layoutOptions);
