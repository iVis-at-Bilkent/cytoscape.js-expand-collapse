const hardcoded_elems = [
  ...
  {
    data: { id: "e11", source: "n3", target: "n4", edgeType: "type3" },
    group: "edges",
  },
  {
  ...
  {
    data: { id: "e30", source: "n3", target: "n3", edgeType: "type3" },
    group: "edges",
  },
  {
    data: { id: "e31", source: "n3", target: "n3", edgeType: "type3" },
    group: "edges",
  },
  {
    data: {
...
function main() {
  ...
  document
    .getElementById("expandAllAndRemove")
    .addEventListener("click", function () {
      api.expandAll();
  ...
  document
    .getElementById("loadInCollapsedState")
    .addEventListener("click", function () {
      if (elements) {
        cy.add(elements);
        api.collapseAll();
        ...
        console.warn(
          "Remove elements first by clicking on 'Expand all and remove' button."
        );
      }
    });
  ...
  function applyMarkovClustering() {
    var clusteringDepth = document.getElementById(
      "clustering-depth-input"
    ).value;
    if (clusteringDepth < 1) return;
    var createdNodeIds = [];
    //cy.startBatch();
    ...
    for (var i = 0; i < clusteringDepth; i++) {
      ...
      for (var j = 0; j < clusters.length; j++) {
        ...
        let parentPos = {
          x: clusterBoundingBox.x1 + clusterBoundingBox.w / 2,
          y: clusterBoundingBox.y1 + clusterBoundingBox.h / 2,
        };
  ...
  document
    .getElementById("apply-markov-clustering")
    .addEventListener("click", applyMarkovClustering);

  function getEdgeOptions() {
    const groupEdgesOfSameTypeOnCollapse =
      document.getElementById("groupEdges").checked;
    const allowNestedEdgeCollapse = document.getElementById(
      "allowNestedEdgeCollapse"
    ).checked;
    return {
      groupEdgesOfSameTypeOnCollapse: groupEdgesOfSameTypeOnCollapse,
      allowNestedEdgeCollapse: allowNestedEdgeCollapse,
    };
  }

  document
    .getElementById("collapseSelectedEdges")
    .addEventListener("click", function () {
  ...
  document
    .getElementById("expandSelectedEdges")
    .addEventListener("click", function () {
      const edges = cy.edges(":selected");
      if (edges.length > 0) {
  ...
  document
    .getElementById("remove-compound")
    .addEventListener("click", function () {
