module.exports = function (cy, api) {
  ...
  function doIt(func) {
    return function (args) {
      ...
      if (args.firstTime) {
        ...
        result.nodes =
          func.indexOf("All") > 0
            ? api[func](args.options)
            : api[func](nodes, args.options);
      } else {
        result.oldData = getNodePositions();
        result.nodes =
          func.indexOf("All") > 0
            ? api[func](secondTimeOpts)
            : api[func](cy.collection(nodes), secondTimeOpts);
  ...
  for (var i = 0; i < actions.length; i++) {
    ...
    else ur.action(actions[i], doIt(actions[i]), doIt(actions[(i + 3) % 6]));
  }
  ...
  function collapseEdgesBetweenNodes(args) {
    ...
    result.options = options;
    if (args.firstTime) {
      var collapseAllResult = api.collapseEdgesBetweenNodes(
        args.nodes,
        options
      );
  ...
  ur.action(
    "collapseEdgesBetweenNodes",
    collapseEdgesBetweenNodes,
    expandEdgesBetweenNodes
  );
  ur.action(
    "expandEdgesBetweenNodes",
    expandEdgesBetweenNodes,
